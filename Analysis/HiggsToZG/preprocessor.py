...
def preprocess(pars, events, AttachSF=False):
    ...
    events["genEle2"]   = ak.num(events.GenEles) >= 2
    events["genMu2"] = ak.num(events.GenMuons) >= 2
    events["genTau2"]  = ak.num(events.GenTaus) >= 2

    events_genEle = events[events.genEle2==True]
    events_genMuons = events[events.genMu2==True]
    events_genTaus = events[events.genTau2==True]

    print(f'# events with gen ele/muo/taus : {len(events_genEle)}/{len(events_genMuons)}/{len(events_genTaus)}')
