...
############################# KEEP THIS BLOCK ##################
    ...
    ev["recoEle"]=ev.Electron[(ev.Electron.cutBased >=2) & (ev.Electron.pt > 20) & (abs(ev.Electron.eta)<2.5)]
    ev["recoMu"]=ev.Muon[(ev.Muon.mediumId==True) & (ev.Muon.pt > 20) & (abs(ev.Muon.eta)<2.4)]
    ev["recoTau"]=ev.Tau[(ev.Tau.idDecayModeNewDMs==True) & (ev.Tau.idAntiMu>=1) & (ev.Tau.idAntiEle>=1)
                                & (ev.Tau.pt > 20) &  (abs(ev.Tau.eta)<2.5)]  ##idAntiMu is important else it gives muons --> ask Anshul if Chao Chen applies it
#ev["recoTau"]=ev.Tau[(ev.Tau.idDecayModeOldDMs==True) & (ev.Tau.pt > 30) &  (abs(ev.Tau.eta)<2.5)]

    ev["Photon","cutBasedBitmap"]=ev.Photon.vidNestedWPBitmap # adding because my nano is v9 and the name is vidNestedWPBitmap
    ev["recoPho"]=ev.Photon[(ev.Photon.cutBasedBitmap >=1) & (ev.Photon.pt > 20) & (abs(ev.Photon.eta)<2.5)]
    ...
    ###################################################
    #h_m_mllg = hist.Hist("Events",hist.Bin("Mttg", "Mllg", 200,100,200))
    ...
    print(f'events with atleast 2 electrons/muons/taus: {len(events_2Ele)}/{len(events_2Mu)}/{len(events_2Tau)}')

    print(f'events with atleast 2 electrons/muons/taus passing dR: {len(events_2Ele_passdR)}/{len(events_2Mu_passdR)}/{len(events_2Tau_passdR)}')

    print(f'events passing all the cuts electrons/muons/taus: {len(events_Ele_final_MW)}/{len(events_Mu_final_MW)}/{len(events_Tau_final_MW)}')
