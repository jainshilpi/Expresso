...
def preprocess(sample,events,AttachSF=True):
    ...
    if histAxisName=="H2TTTG":
        ...
        events=events[(events.ttgevents==1) & (ak.num(events.mytau)==1) & (ak.num(events.myphoton)==1) & ak.num(events.lightlepton)==1]
    elif histAxisName=="H2TTT":
        events=events[(events.ttgevents==0) & (ak.num(events.mytau)==1) & (ak.num(events.myphoton)==1) & ak.num(events.lightlepton)==1]
    else:
        events=events[(ak.num(events.mytau)==1) & (ak.num(events.myphoton)==1) & ak.num(events.lightlepton)==1]
    ev_tau=events.mytau[:,0]
    ev_lepton=events.lightlepton[:,0]
    ev_photon=events.myphoton[:,0]

    events["ehobject"]=ev_tau+ev_lepton
    events["ehgobject"]=ev_tau+ev_lepton+ev_photon

    events["dreh"]=ev_lepton.delta_r(ev_tau)
    events["chargeeh"]=ev_lepton.charge*ev_tau.charge
    events["invarmass"]=events.ehobject.mass
